...
export const ThemeProvider: FC<ThemeProviderProps> = ({ children }) => {
    ...
    const [theme, setTheme] = useState<Theme>(() =>
        typeof window !== 'undefined' && window.matchMedia && window.matchMedia(query).matches ? 'dark' : 'dark'
    );

    useEffect(() => {
        if (window.matchMedia) {
            const listener = (event: MediaQueryListEventMap['change']) => setTheme(event.matches ? 'dark' : 'dark');
